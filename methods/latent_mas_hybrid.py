...
def transfer_via_realignment(
    ...
    if vocab_A != vocab_B:
        ...
        print(
            f"[WARNING] Vocab size mismatch: {vocab_A} vs {vocab_B}, using first {min_vocab} tokens"
        )
    ...
    embeddings_B = embeddings_B_flat.reshape(batch_size, seq_len, dim_B).to(
        original_dtype
    )
...
class LatentMASMethod:
    def __init__(
        ...
        *,
        ...
        if agent_models is None:
            ...
            assert len(agent_models) == len(
                self.agents
            ), "Must specify model for each agent"
    ...
    def _truncate_past(
        self, past_kv: Optional[Tuple], tokens_to_keep: int
    ) -> Optional[Tuple]:
    ...
    def run_batch(self, items: List[Dict]) -> List[Dict]:
        ...
        current_model_name: Optional[str] = (
            None  # Track which model owns current KV cache
        )

        # NEW approach: Track prompts as TEXT and latent hidden states separately
        cumulative_prompts: List[str] = [
            "" for _ in range(batch_size)
        ]  # Accumulate all agent prompts as text
        cumulative_latent_hiddens: Optional[torch.Tensor] = (
            None  # Only RAW latent hidden states (not embeddings)
        )
        ...
        for agent_idx, agent in enumerate(self.agents):
            ...
            if model_switched:
                ...
                print(
                    f"[HYBRID] Cumulative latent hiddens shape: {cumulative_latent_hiddens.shape if cumulative_latent_hiddens is not None else None}"
                )
            ...
            prompts, input_ids, attention_mask, tokens_batch = (
                agent_model.prepare_chat_batch(
            ...
            if agent.role != "judger":
                ...
                if model_switched and cumulative_latent_hiddens is not None:
                    ...
                    prompt_texts_batch = (
                        cumulative_prompts  # List of accumulated prompt text per item
                    )
                    ...
                    prompt_mask = prompt_encoded["attention_mask"].to(
                        agent_model.device
                    )

                    # Get native Model B embeddings for prompts
                    with torch.no_grad():
                        prompt_embeds = agent_model.model.get_input_embeddings()(
                            prompt_ids
                        )
                    ...
                    combined_embeds = torch.cat(
                        [prompt_embeds, transferred_latent_embeds], dim=1
                    )
                    ...
                    past_kv, new_latent_hiddens = (
                        self._capture_hidden_states_from_model(
                            agent_model,
                            wrapped_ids,
                            wrapped_mask,
                            transfer_past_kv,
            ...
            if self.task in ["mbppplus", "humanevalplus"]:
                ...
                try:
                    if pred is None or pred == "":
                        ok = False
                        error_msg = (
                            f"Failed to extract answer from: {final_text[:100]}..."
                        )
                    else:
                        pred_int = int(pred)
                        gold_int = int(gold)
